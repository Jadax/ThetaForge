...
// Cloud Run assigns the service URL at deploy time (it isn't a predictable
// name-based subdomain the way some PaaS free tiers are), so this starts
// blank rather than guessing. Set it once here after your first deploy, or
// just paste it into the panel below — it's saved to localStorage either way.
const DEFAULT_ADVISOR_API = "";
    ...
    <main>
      ...
      <section className="hero">
        ...
        <details>
          ...
          <input className="api" value={apiBase} onChange={(event) => setApiBase(event.target.value)} aria-label="Local Brain address" placeholder="https://your-service-hash.a.run.app" />
          ...
          <p>Use your Cloud Run service URL here for live analysis (printed by <code>deployment/gcp_deploy.ps1</code> after deploy). The token must match <code>ADVISOR_API_TOKEN</code> on that service; it is held for this browser session only. Your IBKR paper-trading Bridge remains local to your trading computer.</p>
        </details>
        {!advisorToken && <p className="error">Enter your Advisor API token to load market analysis.</p>}
        {error && <p className="error">{error}. Check the Cloud Run Advisor URL, then try again.</p>}
      </section>
